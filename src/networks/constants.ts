export const ETH_MAINNET_ARCHIVE='https://v2.archive.subsquid.io/network/ethereum-mainnet'
export const ETH_MAINNET_RPC=process.env.ETH_MAINNET_RPC || ''
export const ETH_MAINNET_CHAIN_ID=1
export const ETH_MAINNET_EMOTES_REPO_ADDRESS='0x31107356a47DD65e832676C1FCBECb0B7C075f5B'
export const ETH_MAINNET_INIT_BLOCK=17770688

export const ETH_SEPOLIA_ARCHIVE='https://v2.archive.subsquid.io/network/ethereum-sepolia'
export const ETH_SEPOLIA_RPC=process.env.ETH_SEPOLIA_RPC || ''
export const ETH_SEPOLIA_CHAIN_ID=11155111
export const ETH_SEPOLIA_EMOTES_REPO_ADDRESS='0x31107356a47DD65e832676C1FCBECb0B7C075f5B'
export const ETH_SEPOLIA_INIT_BLOCK=3932466

export const MOONBEAM_ARCHIVE='https://v2.archive.subsquid.io/network/moonbeam-mainnet'
export const MOONBEAM_RPC=process.env.MOONBEAM_RPC || ''
export const MOONBEAM_CHAIN_ID=1284
export const MOONBEAM_EMOTES_REPO_ADDRESS='0x31107356a47DD65e832676C1FCBECb0B7C075f5B'
export const MOONBEAM_INIT_BLOCK=4065179

export const POLYGON_ARCHIVE='https://v2.archive.subsquid.io/network/polygon-mainnet'
export const POLYGON_RPC=process.env.POLYGON_RPC || ''
export const POLYGON_CHAIN_ID=137
export const POLYGON_EMOTES_REPO_ADDRESS='0x31107356a47DD65e832676C1FCBECb0B7C075f5B'
export const POLYGON_INIT_BLOCK=45477725
